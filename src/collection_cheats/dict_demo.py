...
if __name__ == '__main__':
    ...
    df_dc = collections.defaultdict(lambda: 1)
    df_dc['asdf']
    df_dc['efgh'] = 10
    print(f"dicts from collection {df_dc.items()}")

    df_dct_int = collections.defaultdict(int)
    df_dct_int["asdf"] += 1
    df_dct_int["efgh"] += 10
    print(f"df_dct_int {df_dct_int.items()}")
